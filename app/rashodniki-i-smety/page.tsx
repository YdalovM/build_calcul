...
import { CostEstimateCalculator } from "@/components/calculators/cost-estimate-calculator";
...
import { buildCostEstimateJsonLd } from "@/lib/seo-page-jsonld";

export const metadata: Metadata = buildRouteMetadata("/rashodniki-i-smety");

export default function CostEstimatePage() {
  return (
    <div className="space-y-4">
      <JsonLd data={buildCostEstimateJsonLd()} />

      <PageIntroBand>
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-950 sm:text-3xl">
          {costEstimateContent.pageTitle}
        </h1>
        <p className="mt-3 max-w-3xl text-zinc-600">
          {costEstimateContent.pageDescription}
        </p>
        <p className="mt-3 max-w-3xl text-sm text-zinc-700">
          {costEstimateContent.seoAnswer}
        </p>
      </PageIntroBand>
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="text-xl font-semibold tracking-tight text-zinc-950 sm:text-2xl">
          Частые тематические вопросы по смете
        </h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {costEstimateContent.queryIntents.map((item) => (
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-8">
        <h2 className="text-xl font-semibold tracking-tight text-zinc-950 sm:text-2xl">
          FAQ по расходникам и смете
        </h2>
        <div className="mt-4 space-y-2">
          {costEstimateContent.faq.map((item) => (
            <FaqItem
              key={item.question}
